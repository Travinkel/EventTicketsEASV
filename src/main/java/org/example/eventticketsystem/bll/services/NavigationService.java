...
public class NavigationService implements INavigationService {
     ...
     */
    @Inject
    public NavigationService(Stage primaryStage, Config config) {
    ...
    public void switchScene(String fxmlPath, SessionContext session) {
        logger.info("🚀 Switching scene to: {}", fxmlPath);
    ...
    public Node loadViewNode(String fxmlPath) {
        try {
            ...
            logger.info("✅ View node loaded successfully: {}", fxmlPath);
            return node;
        } catch (IOException e) {
     ...
     */
    public <T> T showDialog(String fxmlPath, Class<T> controllerClass, Consumer<T> initializer) {
        try {
            logger.info("🚪 Opening dialog: {}", fxmlPath);
            FXMLLoader loader = new FXMLLoader(getClass().getResource(fxmlPath));
            loader.setControllerFactory(controllerFactory);
            ...
            logger.info("✅ Dialog closed: {}", fxmlPath);
            return controller;
        } catch (IOException e) {
    ...
    private void setWindowSizeBasedOnScene(String fxmlPath) {
        logger.debug("🔍 Determining window size for scene: {}", fxmlPath);
        switch (fxmlPath) {
    ...
    private void applyGlobalCSS(Scene scene) {
        String cssPath = config.get(Config.Key.GLOBAL_CSS);
        if (cssPath != null && getClass().getResource(cssPath) != null) {
            scene.getStylesheets()
                    .add(Objects.requireNonNull(getClass().getResource(cssPath))
                            .toExternalForm());
            logger.info("✅ Global CSS applied: {}", cssPath);
        } else {
            logger.warn("⚠️ Global CSS not found or invalid: {}", cssPath);
        }
    }
}
